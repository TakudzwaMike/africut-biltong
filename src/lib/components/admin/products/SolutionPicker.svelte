...
<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto">
        {#each allSolutions as solution}
            <label
                class="flex items-start gap-3 p-2 rounded border border-main/10 hover:bg-main/5 cursor-pointer transition-colors"
                class:bg-accent/10={selectedIds.has(solution.id)}
                class:border-accent={selectedIds.has(solution.id)}
            >
                <input
                    type="checkbox"
        ...
        {#if allSolutions.length === 0}
            <p class="text-sm text-main/40 italic col-span-2">No solutions available.</p>
        {/if}
    </div>
</div>
