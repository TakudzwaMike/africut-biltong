	...
	<div class="mx-auto max-w-7xl px-8">
		<div class="grid grid-cols-1 gap-16 lg:grid-cols-3">
			...
			<div class="lg:col-span-2">
				<div class="rounded-2xl bg-white p-8 shadow-xl ring-1 ring-slate-900/5 sm:p-12">
					<h3 class="text-2xl font-bold text-main mb-8">Send us a Message</h3>
					
					{#if form?.message}
						<div class="mb-8 rounded-md p-4 text-sm font-bold {form.success ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}">
							{form.message}
						</div>
					{/if}
					...
					{#if !form?.success}
						<form 
							...
							use:pow
							onpow-solving={() => submissionStatus = 'verifying'}
							onpow-verified={() => {
								if (submissionStatus === 'verifying') submissionStatus = 'idle';
							}}
						>
							{#if solution}
							...
							<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
								<div>
									<label for="firstName" class="block text-sm font-bold text-main/80 mb-2">First Name</label>
									<input
										type="text"
										name="firstName"
										id="firstName"
								...
								<div>
									<label for="lastName" class="block text-sm font-bold text-main/80 mb-2">Last Name</label>
									<input
										type="text"
										name="lastName"
										id="lastName"
							...
							<div>
								<label for="message" class="block text-sm font-bold text-main/80 mb-2">How can we help?</label>
								<textarea
									name="message"
									id="message"
									rows="5"
							...
							<div class="pt-4">
								...
								>
									{#if submissionStatus === 'verifying'}
										<Icon icon="mdi:shield-sync" class="animate-spin" />
										Verifying Security...
			...
			<div class="space-y-8">
				...
				<div class="rounded-xl bg-main p-8 text-light shadow-xl">
					<h3 class="text-xl font-bold text-accent mb-6">Quick Contact</h3>
					<div class="space-y-4">
						<div class="flex items-center gap-4">
							...
							<div>
								<p class="text-xs text-light/60 uppercase font-bold">Email Us</p>
								<a href="mailto:hello@vision-ai.tech" class="font-medium hover:text-accent">hello@vision-ai.tech</a>
							</div>
						</div>
						<div class="flex items-center gap-4">
							<div class="flex h-10 w-10 items-center justify-center rounded-full bg-white/10">
							...
							<div>
								<p class="text-xs text-light/60 uppercase font-bold">Response Time</p>
				...
				{#if data.locations.length > 0}
					<div>
						<h3 class="text-lg font-bold text-main mb-4 flex items-center gap-2">
							<Icon icon="mdi:map-marker-multiple" class="text-accent" />
							Our Offices
						</h3>
						<div class="space-y-4">
							{#each data.locations as loc}
								<div class="rounded-xl border border-main/10 bg-white p-6 shadow-sm transition hover:border-accent hover:shadow-md">
									<h4 class="font-bold text-main">{loc.countryName}</h4>
									<p class="mt-2 text-sm text-main/70">{loc.address}</p>
									{#if loc.phoneNumber}
										<a
											href="tel:{loc.phoneNumber.replace(/\s/g, '')}"
											class="mt-4 inline-flex items-center gap-1 text-sm font-bold text-accent hover:underline"
										>
											<Icon icon="mdi:phone" /> {loc.phoneNumber}
										</a>
									{/if}
								</div>
							{/each}
