...
<div class="relative z-10 bg-slate-50 py-24">
    <div class="mx-auto max-w-7xl px-8">
        ...
        <!-- Toolbar -->
        <div class="mb-12 flex items-center justify-between border-b border-slate-200 pb-4">
            <p class="text-sm font-bold text-slate-500 uppercase tracking-wide">
                {data.products.length} Products
            </p>
            <div class="flex items-center gap-2 text-sm font-bold">
                <span class="text-slate-400">Currency:</span>
                <button onclick={() => currency.setCurrency('USD')} class:text-accent={$currency === 'USD'}>USD</button>
                <span class="text-slate-300">|</span>
                <button onclick={() => currency.setCurrency('ZAR')} class:text-accent={$currency === 'ZAR'}>ZAR</button>
            </div>
        </div>
            ...
            {#each data.products as product}
                ...
                <a href={`/products/${product.slug}`} class="group relative flex flex-col overflow-hidden rounded-xl bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <div class="flex flex-1 flex-col p-6">
                        <h3 class="text-lg font-bold text-main group-hover:text-accent transition-colors">{product.name}</h3>
        ...
        {#if data.products.length === 0}
            <div class="py-20 text-center text-main/50">
                <Icon icon="mdi:package-variant-closed" width="64" class="mx-auto mb-4 opacity-50" />
