<script>
    ...
    import { currency, currencyRates } from "$lib/stores/currency.js";
    ...
    let allImages = $derived(
        [product.featuredImage, ...product.images.map((i) => i.media)].filter(
            Boolean,
        ),
    );
    ...
    let pricing = $derived.by(() => {
        ...
        const currentCents = isZar
            ? Math.round(
                  selectedVariant.effectivePriceUsd * $currencyRates.USD_TO_ZAR,
              )
            : selectedVariant.effectivePriceUsd;

        const originalCents =
            isZar && selectedVariant.compareAtPriceUsd
                ? Math.round(
                      selectedVariant.compareAtPriceUsd *
                          $currencyRates.USD_TO_ZAR,
                  )
                : selectedVariant.compareAtPriceUsd;
    ...
    let stockStatus = $derived.by(() => {
        ...
        if (product.type === "physical") {
            ...
            if (selectedVariant.stock > 0)
                return {
                    label: `Low Stock (${selectedVariant.stock})`,
    ...
    let productSchema = $derived({
        ...
        offers: {
            ...
            availability:
                product.type !== "physical" || selectedVariant?.stock > 0
...
<Seo
    ...
    description={product.shortDescription}
    imageUrl={product.featuredImage?.displayUrl ||
        product.featuredImage?.originalUrl}
    ogType="product"
/>
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div
                    class="aspect-[4/3] w-full overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm relative group"
                >
                    {#if activeImage}
                        <Image
                    ...
                    >
                        <span
                            class="bg-white/90 backdrop-blur text-main text-xs font-bold px-3 py-1 rounded-full shadow-sm border border-slate-100 uppercase tracking-wider"
                        >
                            {product.type}
                        </span>
                        {#if pricing.isOnSale}
                            <span
                                class="bg-accent text-main text-xs font-bold px-3 py-1 rounded-full shadow-sm uppercase tracking-wider animate-pulse"
                            >
                                {pricing.badge}
                            </span>
                        {/if}
            ...
            <div class="lg:col-span-5">
                <div class="sticky top-24 space-y-8">
                    ...
                    <div>
                        <h1
                            class="text-3xl font-bold text-main tracking-tight sm:text-4xl"
                        >
                            {product.name}
                        </h1>
                    ...
                    <!-- Price & Stock -->
                    <div
                        class="flex items-center justify-between border-y border-slate-200 py-6"
                    >
                        <div>
                            <p
                                class="text-sm text-slate-500 font-bold uppercase tracking-wider mb-1"
                            >
                                Price
                            </p>
                            <div class="flex items-baseline gap-3">
                                <p
                                    class="text-4xl font-bold text-main {pricing.isOnSale
                                        ? 'text-accent'
                                        : ''}"
                                >
                                    {pricing.current}
                        ...
                        <div class="text-right">
                            ...
                            <div
                                class="flex items-center justify-end gap-1 {stockStatus.color}"
                            >
                    ...
                    {#if product.variants.length > 1}
                        <div>
                            <label
                                for="variant"
                                class="block text-sm font-bold text-main mb-3"
                                >Configuration</label
                            >
                    ...
                    {#if product.features.length > 0}
                        <div
                            class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-900/5 mt-6"
                        >
                            <h3
                                class="text-sm font-bold text-main uppercase tracking-wider mb-4"
                            >
                                Highlights
                            </h3>
                            <ul class="space-y-3">
                                {#each product.features as feature}
                                    ...
                                    >
                                        <Icon
                                            icon={feature.icon ||
                                                "mdi:check-circle"}
                                            class="mt-0.5 text-accent flex-shrink-0"
                                            width="18"
                                        />
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8 mt-24">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-8">
                <h2
                    class="text-2xl font-bold text-main mb-8 border-b border-slate-200 pb-4"
                >
                    Product Details
                </h2>
            ...
            {#if product.solutions.length > 0}
                <div class="lg:col-span-4">
                    <div
                        class="rounded-xl bg-main text-white p-8 shadow-xl sticky top-24"
                    >
                        <h3 class="text-xl font-bold mb-2">
                            Integrated Ecosystem
                        </h3>
                        <p class="text-white/70 text-sm mb-6">
                            This product is optimized for the following Vision
                            AI solutions.
                        </p>
                        ...
                        <div class="space-y-4">
                            {#each product.solutions as link}
                                ...
                                <a
                                    ...
                                    class="flex items-center gap-4 rounded-lg bg-white/10 p-3 hover:bg-white/20 transition-colors group"
                                >
                                    <div
                                        class="h-12 w-12 flex-shrink-0 rounded bg-white/5 overflow-hidden"
                                    >
                                        {#if sol.featuredImage}
                                            <Image
                                                src={sol.featuredImage
                                                    .thumbnailUrl ||
                                                    sol.featuredImage
                                                        .originalUrl}
                                    ...
                                    <div>
                                        <p
                                            class="font-bold text-white text-sm group-hover:text-accent transition-colors"
                                        >
                                            {sol.solutionName}
                                        </p>
                                        <p
                                            class="text-xs text-white/50 uppercase tracking-wider font-bold mt-1"
                                        >
                                            View Solution →
                                        </p>
                                    </div>
